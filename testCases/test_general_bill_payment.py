...
class Test_004_Electric:
    ...
    def test_008_education(self, setup):
        ...
        self.rows = XLUtils.getRowCount(self.path, 'General Bill')

        ## Electricity
        self.bill.clickGeneralBill()
        self.bill.selectUSDaccount()

        for r in range(2, self.rows+1):
            self.biller = XLUtils.readData(self.path, 'General Bill', r, 1)
            self.bill.clickBillerDropDown()
            self.bill.choose_a_general_bill_biller(self.biller)
            self.log.info("%s Found", self.biller)

            self.consumer = XLUtils.readData(self.path, 'General Bill', r, 2)
