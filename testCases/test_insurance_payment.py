...
class Test_004_Electric:
    ...
    def test_006_education(self, setup):
        ...
        self.rows = XLUtils.getRowCount(self.path, 'Insurance')
        ...
        for r in range(2, self.rows+1):
            self.biller = XLUtils.readData(self.path, 'Insurance', r, 1)
            self.bill.clickBillerDropDown()
            self.bill.choose_an_insurance_biller(self.biller)
            self.log.info("%s Found", self.biller)

            self.consumer = XLUtils.readData(self.path, 'Insurance', r, 2)
