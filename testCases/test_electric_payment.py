...
class Test_004_Electric:
    ...
    def test_003_electric(self, setup):
        ...
        ## Electricity
        self.bill.clickElectricity()
        self.bill.selectUSDaccount()

        for r in range(2, self.rows+1):
            self.biller = XLUtils.readData(self.path, 'Electric', r, 1)
            self.bill.clickBillerDropDown()
            self.bill.choose_an_electric_biller(self.biller)
            self.log.info("%s Found", self.biller)

            self.consumer = XLUtils.readData(self.path, 'Electric', r, 2)
