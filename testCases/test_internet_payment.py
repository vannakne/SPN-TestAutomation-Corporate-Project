...
class Test_004_Electric:
    ...
    def test_006_education(self, setup):
        ...
        for r in range(2, self.rows+1):
            self.biller = XLUtils.readData(self.path, 'Internet', r, 1)
            self.bill.clickBillerDropDown()
            self.bill.choose_an_internet_biller(self.biller)
            self.log.info("%s Found", self.biller)

            self.consumer = XLUtils.readData(self.path, 'Internet', r, 2)
