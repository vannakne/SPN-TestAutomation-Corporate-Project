...
class Test_Mobile_TopUP:
    ...
    def test_pinless(self, setup):
        ...
        for r in range(2, self.rows + 1):
            ...
            self.mobile.enter_phone_number(self.number)
            self.amount = XLUtils.readData(self.path, self.sheetName, r, 2)
            self.mobile.clickTopUP_amount_Dropdown()
            self.mobile.choose_an_amount(self.amount)
            self.mobile.clickRemark()
            sleep(1)
            try:
